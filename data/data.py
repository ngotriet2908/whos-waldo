    ...
    def __init__(self, img_dir, conf_th=0.2, max_bb=100, min_bb=10, num_bb=36,
        ...
        if self.name2nbb is None:
            if compress:
                ...
                db_name = 'all'
                self.name2nbb = json.load(open(f'{img_dir}/name2nbb.json'))
...
class TxtTokLmdb(object):
    def __init__(self, db_dir, max_txt_len=60):
        if max_txt_len == -1:
            ...
            self.id2len = {
                ...
                if len_ <= max_txt_len -1 # for the extra cls token
            }
        self.db_dir = db_dir
    ...
    @property
    def id2category(self):
        id2category = json.load(open(f'{self.db_dir}/id2category.json'))
        return id2category
